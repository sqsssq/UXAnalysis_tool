...
<template>
    <div class="frameworkBody">
        <div style="height: 80px;">
            ...
            <el-switch v-model="recommendTag" class="mb-2" style="--el-switch-on-color: #409eff; --el-switch-off-color: #409eff" active-text="Secondary" inactive-text="Primary" />
        </div>
        <div ref="wholeWidth" id="problem_tag" style="height: calc(100% - 80px)">
            <el-dialog v-model="addPoint" :title="'Add ' + add_tag_level + ' Class.'" width="15%" height="100px" :append-to="'#problems_tag'" :modal="false" :class="'add_dialog'">
                <div v-loading="loadingTag" element-loading-background="#454647" style="width: 100%; height: 170px; margin-top: -10px;">
    
                    <h3 style="color: white; text-align: left;">Name</h3>
                    <span>
                                            <el-input v-model="tag_name" placeholder="Please input"  :input-style="{ 'font-size': '18px' }"/>
                                        </span>
                    <h3 style="color: white; text-align: left;">Definition</h3>
                    <span>
                ...
                <div style="padding: 20px 0px 10px 0px;">
                    ...
                    <el-button @click="addPoint = false">Cancel</el-button>
                </div>
            </el-dialog>
            <el-dialog v-model="optimizeShowTag" :title="'Classification Optimization'" width="22%" height="100px" :append-to="'#problems_tag'" :modal="false" :class="'optimize_dialog'">
                <div v-loading="optimizeLoading" element-loading-background="#454647" style="width: 100%; height: 400px; margin-top: -10px; overflow-y: auto;">
                    <div v-for="(d, i) in calcUnion" :keys="'union_tag_' + i" style="width: 100%; background-color: rgba(0, 0, 0, .3); padding: 10px 10px; border-radius: 5px; margin-top: 10px;">
                        <h2 style="color: white; text-align: left;">{{ 'Merging ' + (d.level == 1 ? 'Primary' : 'Secondary') + ' Classification' }}</h2>
                        <h3 style="color: white; text-align: left;">Original Class.</h3>
                        ...
                        <h3 style="color: white; text-align: left; margin-top: 10px;">New Class. Name</h3>
                        <span>
                                                            <el-input v-model="d.combined_name" placeholder="Please input" :input-style="{ 'font-size': '18px' }" />
                                                        </span>
                        <h3 style="color: white; text-align: left; margin-top: 10px;">New Class. Definition</h3>
                        <span>
                        ...
                        <div style="margin-top: 15px">
                            ...
                            <el-button type="primary" @click="unionTag(d, 1)">Yes</el-button>
                            <el-button type="danger" @click="unionTag(d, 0)">Cancel</el-button>
                        </div>
                    </div>
                </div>
            </el-dialog>
            <div style="text-align: left; font-size: 24px; color: white; font-weight: bold; height: 40px; display: flex; justify-content: space-between;">
                <span style="display: flex;">
                                            <span>Usability Problem Classification</span> &nbsp;
                <!-- <span>
                                                    <el-popover
                                                        placement="bottom"
                                                        title="标签解释"
            ...
            <span>
                <el-button type="primary" @click="showOptimize()">Optimize</el-button>
            </span>
            </div>
            <div style="text-align: left; padding-left: 24px; height: 45px;width: 100%;">
                <span>
                        <el-checkbox label="All" size="large" v-model="selectAll" />
                    </span>
                <span style=" position: absolute;top: 12px; right: 0px;">
                        <el-button style="margin-top: -10px;" type="primary" @click="showDialog(1, dataSource)">Add Primary</el-button>
                    </span>
            </div>
...
export default {
    ...
    methods: {
        ...
        showDialog(tag_type, data) {
            ...
            if (tag_type == 1) {
                this.add_tag_level = 'Primary';
            } else {
                this.add_tag_level = 'Secondary';
            }
        },
        addTag() {
            let id_cnt = -1;
            ...
            //     tag: this.tag_name,
            //     id: id_cnt,
            //     parent_id: this.add_tag_level == 'Primary' ? 0 : this.selectData.id,
            //     level: this.add_tag_level == 'Primary' ? 1 : 2,
            //     category: this.dataSource,
            //     info: this.all_data,
            //     test: 0
            // };
