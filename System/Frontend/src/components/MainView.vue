                ...
                <div id="dialogDiv" ref="dialogDiv" :style="{width: '100%', height: elHeight * .5 + 'px', color: 'white', textAlign: 'left', overflow: 'auto'}">
                    ...
                    <div @click="focusTag('main')" :style="{display: 'flex', flexWrap: 'wrap', backgroundColor: showLevelTag.showFirstTag == true ? '#666666AA' : '#45464700', padding: '0px 3px 0px 3px',  'border-top-left-radius': '10px', 'border-top-right-radius': '10px', cursor: 'pointer'}">
                        <div v-if="info_data.tag.length > 0" style="display: flex; flex-wrap: wrap;">
                            <div v-for="d in info_data.tag" :key="'type_tag' + d" class="align-class" :style="{backgroundColor: colorMap[dataSource[d - 1].color] + '40', padding: '3px 8px 3px 5px', 'border-radius': '100px', marginRight: '10px', marginTop: '5px', 'font-size': '15px'}">
                                <div :style="{backgroundColor: colorMap[dataSource[d - 1].color], width: '15px', height: '15px', 'margin': '0px 3px 0px 3px', borderRadius: '20px'}">
                                </div>
                                <div>
                    ...
                    <el-collapse-transition>
                        <div v-if="showLevelTag.showFirstTag" style="background-color: #666666AA; padding: 3px 3px 8px 3px; border-bottom-left-radius: 10px; border-bottom-right-radius: 10px;">
                            ...
                            <div style="display: flex; flex-wrap: wrap;">
                                <div v-for="d in dataSource" :key="'type_tag' + d" class="align-class" :style="{backgroundColor: colorMap[d.color] + '40', padding: '3px 8px 3px 5px', 'border-radius': '100px', marginRight: '10px', marginTop: '5px', 'font-size': '15px', cursor: 'pointer'}"
                                    @click="selectTag(info_data.tag, d.id, info_data)">
                                    <div :style="{backgroundColor: colorMap[d.color], width: '15px', height: '15px', 'margin': '0px 3px 0px 3px', borderRadius: '20px'}">
                        ...
                        <div v-for="(d, i) in info_data.tag" :key="'type_first_tag' + d">
                            ...
                            <div @click="focusTag(i)" :style="{ display: 'flex', flexWrap: 'wrap', backgroundColor: showLevelTag.showSecondTag[i] == true ? '#666666AA' : '#45464700', padding: '0px 3px 0px 3px',  'border-top-left-radius': '10px', 'border-top-right-radius': '10px', cursor: 'pointer'}">
                                <div v-if="typeof info_data.second_tag[d] != 'undefined' && info_data.second_tag[d].length > 0" style="display: flex; flex-wrap: wrap;">
                                    <div v-for="(dd, ii) in info_data.second_tag[d]" :key="'type_second_tag' + dd" class="align-class" :style="{backgroundColor: colorMap[dataSource[d - 1].children[dd - 1].color] + '40', padding: '3px 8px 3px 5px', 'border-radius': '100px', marginRight: '10px', marginTop: '5px', 'font-size': '15px'}">
                                        <div :style="{backgroundColor: colorMap[dataSource[d - 1].children[dd - 1].color], width: '15px', height: '15px', 'margin': '0px 3px 0px 3px', borderRadius: '0px'}">
                                        </div>
                                        <div>
                            ...
                            <el-collapse-transition>
                                <div v-if="showLevelTag.showSecondTag[i]" style="background-color: #666666AA; padding: 3px 3px 8px 3px; border-bottom-left-radius: 10px; border-bottom-right-radius: 10px;">
                                    ...
                                    <div style="display: flex; flex-wrap: wrap;">
                                        <div v-for="dd in dataSource[d - 1].children" :key="'type_tag' + d" class="align-class" :style="{backgroundColor: colorMap[dd.color] + '40', padding: '3px 8px 3px 5px', 'border-radius': '100px', marginRight: '10px', marginTop: '5px', 'font-size': '15px', cursor: 'pointer'}"
                                            @click="selectTag(info_data.second_tag[d], dd.cnt, 0)">
                                            <div :style="{backgroundColor: colorMap[dd.color], width: '15px', height: '15px', 'margin': '0px 3px 0px 3px', borderRadius: '0px'}">
                    ...
                    <div style="width: 100%; position: absolute; top: 1px;">
                        <div v-for="(tag_d, tag_i) in calcMarkerData" :key="'marker' + tag_i" :style="{position: 'absolute', left: 'calc('+tag_d.percentage + '% - 8px)'}">
                            <div v-if="selectShowLevel == 1">
                                <div v-for="(d, i) in tag_d.tag" :key="'tag' + i" :style="{ position: 'absolute', top: (-20 * i) + 'px',
                                                                                                                        height: '16px', width: '16px', backgroundColor: colorMap[dataSource[d - 1].color], borderRadius: '10px',
                                ...
                                <div v-for="(d, i) in tag_d.tag" :key="'tag' + i" :style="{ position: 'absolute', top: (-20 * i) + 'px', height: '16px', width: '16px', borderRadius: '0px', border: '0px solid #777'}">
                                    <div v-if="typeof tag_d.second_tag[d] != 'undefined' && tag_d.second_tag[d].length > 0">
                                        <div v-for="(dd, ii) in tag_d.second_tag[d]" :key="'sec_tag' + ii" :style="{ position: 'absolute', top: (-20 * (i * tag_d.second_tag[d].length + ii)) + 'px', left: '0px', height: '16px', width: '16px', borderRadius: '0px', border: '0px solid #777'}">
                                            <div :style="{position: 'absolute', top: '0px', left: '-9px', height: '16px', width: '16px', backgroundColor: colorMap[dataSource[d - 1].color], borderRadius: '10px', border: '1px solid #777'}"></div>
                                            <div :style="{position: 'absolute', top: '0px', left: '9px', height: '16px', width: '16px', backgroundColor: colorMap[dataSource[d - 1].children[dd - 1].color], borderRadius: '0px', border: '1px solid #777'}"></div>
                                        </div>
                                    </div>
                                </div>
                            </div>
...
export default {
    ...
    mounted() {
        ...
        dataStore.$subscribe((mutations) => {
            ...
            if (mutations.events.key == 'selectShowLevel') {
                this.selectShowLevel = mutations.events.newValue;
            }
        });
    },
    watch: {
